...
for dataset in $DATASETS; do
  python gridsearch.py --model=GCN --hidden_channels=64 --dataset=$dataset --runs=$GRIDSEARCH_RUNS
done
for dataset in $DATASETS; do
  python gridsearch.py --model=GAT --hidden_channels=8 --heads=8 --dataset=$dataset --runs=$GRIDSEARCH_RUNS
done
